...
export default function Reminders() {
  const { items, loading, reload, refresh, create, remove, error} = useResource<Reminder>('/api/reminders')
  ...
  async function save(body: Partial<Reminder>) { await create(body); refreshAttn(); toast('Reminder set'); setOpen(false) }
      ...
      {Object.entries(groups).map(([mod, rows]) => (
        <div key={mod}>
          ...
          <div className="list" style={{ gap: 8 }}>
            {rows.map((r) => <ReminderRow key={r.id} r={r} onToggle={() => toggle(r.id)} onDelete={() => { remove(r.id); toast('Deleted') }} />)}
          </div>
        </div>
      ))}
      {done.length > 0 && <>
        ...
        <div className="list" style={{ gap: 8 }}>
          {done.map((r) => <ReminderRow key={r.id} r={r} onToggle={() => toggle(r.id)} onDelete={() => { remove(r.id); toast('Deleted') }} />)}
        </div>
      </>}
      {open && <ReminderForm onSave={save} onClose={() => setOpen(false)} />}
    </ModuleScreen>
  )
}
...
function ReminderForm({ onSave, onClose }: { onSave: (b: Partial<Reminder>) => void; onClose: () => void }) {
  const [title, setTitle] = useState('')
  const [module_ref, setMod] = useState<string>('general')
  ...
  return (
    <Sheet title="New reminder" onClose={onClose}>
      ...
      <div className="row2">
        ...
        <Field label="Repeat"><select className="select" value={recurrence} onChange={(e) => setRec(e.target.value)}><option value="none">Once</option><option value="monthly">Monthly</option><option value="yearly">Yearly</option></select></Field>
      </div>
      <Field label="Due date"><input className="input" type="date" value={due_date} onChange={(e) => setDue(e.target.value)} /></Field>
      <button className="btn block" onClick={() => onSave({ title, module_ref: module_ref === 'general' ? null : module_ref, due_date, recurrence })} disabled={!title.trim()}>Set reminder</button>
    </Sheet>
  )
}
