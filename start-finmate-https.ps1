...
# Builds the frontend and serves FinMate (app + API) over HTTPS on port 8443,
# for use from an iPhone / iPad / other device on the same Wi-Fi as this PC.
# Prerequisite: MySQL must be running (the normal "Start FinMate" launcher starts it).
...
Write-Host " FinMate is serving over HTTPS on port 8443." -ForegroundColor Green
Write-Host ""
...
Write-Host "       About > Certificate Trust Settings > turn ON 'FinMate Local CA')"
